...
async function initDashboard() {
    try {
        if (!window.siteSettings) window.siteSettings = await api.getSettings();
...
async function saveSettings() {
    ...
    try {
        ...
        // Reload global settings so currency updates everywhere immediately
        const newSettings = await api.getSettings();
        window.siteSettings = newSettings;
