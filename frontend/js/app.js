...
/** Promise used by other scripts to wait for settings before rendering */
window.whenSettingsReady = (async () => {
    try {
...
async function init() {
    const settings = await window.whenSettingsReady;
    if (settings) {
